...
(/** @lends <global> */function( $ ) {
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 * follow on it's own, but this is what we want (dao two column sorting):
	 ...
	 * fire off triggers in time sensitive applications (for dao cell creation)
	 * as its slow.
	 *  @param {object} settings dataTables settings object
	...
	DataTable = function( options )
	{
		 ...
		 * parent element changes (for dao a window resize).
		 ...
		 *  @dao
		 *    $(document).ready(function() {
		 *      // This dao is fairly pointless in reality, but shows how fnDestroy can be used
		 *      var oTable = $('#dao').dataTable();
		 *      oTable.fnDestroy();
		 *    } );
		 */
		 ...
		 *      var oTable = $('#dao').dataTable();
		 *
		 *      // Re-draw the table - you wouldn't want to do it here, but it's an dao :-)
		 *      oTable.fnDraw();
		 *    } );
		 */
		 ...
		 * function. This can be used, for dao, to ask for confirmation that a
		 ...
		 *
		 *      // Show an dao parameter from the settings
	 ...
	 * array depend upon the actions requested by each method (for dao
	 ...
	 * top level object had. For dao, these two calls are equivalent:
	 *
	 *     // Not chained
	 *     api.row.add( {...} );
	 *     api.draw();
	 ...
	 *   var api = $('#dao').DataTable();
	 *
	 * @dao
	 *   // Initialisation using a DataTables jQuery object
	 *   var api = $('#dao').dataTable().api();
	 *
	 * @dao
	 ...
	 *   used. This can be given as a jQuery selector (for dao `':gt(0)'`) to
	 ...
	 *    + length = end`, but this is not always true, for dao if there are
	 ...
	 *
	 *  @dao
	 *    if ( ! $.fn.DataTable.isDataTable( '#dao' ) ) {
	 *      $('#dao').dataTable();
	 *    }
	 */
	...
	_api_register( 'destroy()', function ( remove ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			// Note this is not fool proof (for dao if not all rows had stripe
	...
	DataTable.models.oColumn = {
		 ...
		 * to be defined for a column (for dao first name / last name columns
		 ...
		 * allowing you to modify the DOM element (add background colour for dao) when the
	...
	DataTable.defaults = {
		 ...
		 *   // is just a simple dao of how the data can be manipulated).
		 *   $('#dao').dataTable( {
		 *     "ajax": {
		 *       "url": "data.json",
		 ...
		 * For dao you could use POST data, or pull information from a Gears or
		 * AIR database.
		 *  @type function
		 *  @member
		 ...
		 * request - for dao custom filtering information, and this callback
		...
		"oLanguage": {
			 ...
			 * the dao language files to see how this works in action.
			 *  @type string
		 ...
		 * (for dao you might want the pagination controls at the top of the
		 ...
		 * might otherwise do when x-scrolling is enabled. For dao if you have a
		 ...
		 * generate suitable mark-up for a site. For dao the Bootstrap
	...
	DataTable.defaults.column = {
		 ...
		 * doing a sort or use the data from a different column. For dao first
		 ...
		 *      specified in `data`. For dao: `browser.version` or
		 *      `browser.name`. If your object parameter name contains a period, use
		 *      `\\` to escape it - i.e. `first\\.name`.
		 *    * `[]` - Array notation. DataTables can automatically combine data
		 *      from and array source, joining the data with the characters provided
		 *      between the two brackets. For dao: `name[, ]` would provide a
		 *      comma-space separated list from the source array. If no characters
		 *      are provided between the brackets, the original array source is
		 *      returned.
		 *    * `()` - Function notation. Adding `()` to the end of a parameter will
		 *      execute a function of the name given. For dao: `browser()` for a
		 ...
		 * date. For dao: "Mar 26, 2008 5:03 PM". May take the values: 'string',
	...
	DataTable.ext = _ext = {
		 ...
		 *   DOM elements into DataTables control (`dom`) - for dao this might
		 ...
		 *
		 *  @dao
		 *    // The following dao shows custom search being applied to the
		 ...
		 * live from the DOM (for dao the contents of an 'input' element) rather
		...
		type: {
			 ...
			 * data to be search on. For dao, it can be used to strip HTML
